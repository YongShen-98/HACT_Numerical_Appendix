...
s = 2; %CRRA utility with parameter s       u=x^(1-s)/1-s
...
for n=1:maxit
    V = v;
    % forward difference   
    % page 2 equation 8  forward difference or back difference with a
    
    dVf(1:I-1,:) = (V(2:I,:)-V(1:I-1,:))/da;
    dVf(I,:) = 0; %will never be used
    % backward difference
    dVb(2:I,:) = (V(2:I,:)-V(1:I-1,:))/da;
    dVb(1,:) = (z + r.*amin).^(-s); %state constraint boundary condition    公式（7）
    
    I_concave = dVb > dVf; %indicator whether value function is concave (problems arise if this is not the case)
    
    %consumption and savings with forward difference     u=x^(1-s)/1-s  => (u')的反函数: x^(-1/s)     
    cf = dVf.^(-1/s);
    muf = zz + r.*aa - cf;
    ...
    dV_Upwind = dVf.*If + dVb.*Ib + dV0.*I0; %important to include third term      equation (11)
    
    c = dV_Upwind.^(-1/s);                       % page 4 summmary of algorithm step 2
    V_switch = [V(:,2),V(:,1)];
    Vchange = c.^(1-s)/(1-s) + dV_Upwind.*(zz + r.*aa - c) + ones(I,1)*la.*(V_switch - V) - rho.*V;   % equation (10) Vchange means the difference between vn+1 and vn with delta
